...
auto CalcN(uint h, uint k) { return 6 * (h * h + k * k); }

auto CalcD(uint h, uint k) { return h + k - 1; }
...
template <typename T>
inline size_t Polyhedron<T>::Id(uint x, uint y, uint f) const {
  return id_convert_map_.at(Index(x, y, f, d_));
}
